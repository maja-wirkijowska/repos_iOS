...
struct ScoreContainerModifier: ViewModifier {
  func body(content: Content) -> some View {
    content
      ...
      .frame(minWidth: 128)
      .background(
        Capsule().foregroundColor(colorTransparentBlack)
      )
  }
}
...
struct BetCapsuleModifier: ViewModifier {
  func body(content: Content) -> some View {
    content
      .background(
        Capsule()
          .fill(LinearGradient(gradient: pinkPurpleGradient, startPoint: .top, endPoint: .bottom))
      ) //: button - inner capsure
      .padding(3)
      .background(
        Capsule()
          .fill(LinearGradient(gradient: purplePinkGradient, startPoint: .bottom, endPoint: .top))
          .modifier(ShadowModifier())
      ) //: button - outer capsule
  }
}
...
struct CasinoChipsModifier: ViewModifier {
  func body(content: Content) -> some View {
    content
      .scaledToFit()
      .frame(height: 64)
      .animation(.default, value: casinoChips)
      .modifier(ShadowModifier())
  }
}
